...
namespace {
    class CDigitalKashAddressVisitor : public boost::static_visitor<bool> {
    private:
        CDigitalKashAddress *addr;
    public:
        CDigitalKashAddressVisitor(CDigitalKashAddress *addrIn) : addr(addrIn) { }
...
bool CDigitalKashAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CDigitalKashAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CDigitalKashAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CDigitalKashAddressVisitor(this), dest);
}

bool CDigitalKashAddress::IsValid() const {
...
CTxDestination CDigitalKashAddress::Get() const {
    if (!IsValid())
...
bool CDigitalKashAddress::GetKeyID(CKeyID &keyID) const {
...
bool CDigitalKashAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CDigitalKashSecret::SetKey(const CKey& vchSecret) {
...
CKey CDigitalKashSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CDigitalKashSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CDigitalKashSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CDigitalKashSecret::SetString(const std::string& strSecret) {
