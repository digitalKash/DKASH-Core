...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "DigitalKash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "DigitalKash is downloading blocks...");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        for (int i = 0; i < 128/4; i++)
            ((unsigned int*)pdata)[i] = ByteDigitalKasherse(((unsigned int*)pdata)[i]);
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "DigitalKash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "DigitalKash is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (pindexBest->GetBlockTime() > 0)
    {
        if (pindexBest->nHeight > 200)
        {
            ...
            std::string devpayee2 = Params().DevOpsAddress();
