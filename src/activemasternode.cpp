...
//
// Bootup the masternode, look for a 2,000,000 DKASH input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in DigitalKash.conf */
...
{
    CDigitalKashAddress address(collateralAddress);
