...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(DigitalKashUnits::DKASH)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(fUseDarkTheme ? QColor(255, 255, 255) : foreground);
        QString amountText = DigitalKashUnits::formatWithUnit(unit, amount, true);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchOnlyStake, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(DigitalKashUnits::formatWithUnit(nDisplayUnit, balance));
    ui->labelStake->setText(DigitalKashUnits::formatWithUnit(nDisplayUnit, stake));
    ui->labelUnconfirmed->setText(DigitalKashUnits::formatWithUnit(nDisplayUnit, unconfirmedBalance));
    ui->labelImmature->setText(DigitalKashUnits::formatWithUnit(nDisplayUnit, immatureBalance));
    ui->labelTotal->setText(DigitalKashUnits::formatWithUnit(nDisplayUnit, balance + stake + unconfirmedBalance + immatureBalance));
    //ui->labelWatchAvailable->setText(DigitalKashUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance));
    //ui->labelWatchStake->setText(DigitalKashUnits::floorWithUnit(nDisplayUnit, watchOnlyStake));
    //ui->labelWatchPending->setText(DigitalKashUnits::floorWithUnit(nDisplayUnit, watchUnconfBalance));
    //ui->labelWatchImmature->setText(DigitalKashUnits::floorWithUnit(nDisplayUnit, watchImmatureBalance));
    //ui->labelWatchTotal->setText(DigitalKashUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance + watchOnlyStake + watchUnconfBalance + watchImmatureBalance));
...
void OverviewPage::updateWatchOnlyLabels(bool showWatchOnly)
{
    ...
    //ui->labelDKASHtotalimg->setVisible(showWatchOnly);    // show watch-only balance separator line
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("DKASH")
    updateDisplayUnit();
}
...
{
    DigitalKashGUI::gotoHistoryPage_static();
}

void OverviewPage::setLockIconLocked(){
    ui->lockstatusLabel->setPixmap(QPixmap(":/icons/lock_closed_toolbar"));
}

void OverviewPage::setLockIconUnlocked(){
    ui->lockstatusLabel->setPixmap(QPixmap(":/icons/lock_open_toolbar"));
}
