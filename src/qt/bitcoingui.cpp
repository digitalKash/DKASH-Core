 ...
 * The DigitalKash Developers 2018-2020
 */
...
DigitalKashGUI::DigitalKashGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
    toolbar(0),
    ...
    nWeight(0)
{
    ...
    setWindowTitle(tr("DigitalKash") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    ...
    setObjectName("DigitalKash");
    setStyleSheet("#DigitalKash { background-image: url(:/images/background_light); }");//background-color: #e6e6e6; color: #333333;
...
void DigitalKashGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a DigitalKash address"));
    ...
    aboutAction = new QAction(QIcon(fUseDarkTheme ? ":/icons/dark/bitcoin-dark" : ":/icons/bitcoin"), tr("&About DigitalKash"), this);
    aboutAction->setToolTip(tr("Show information about DigitalKash"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for DigitalKash"));
    ...
    editConfigAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit DigitalKash.conf"), this);
    editConfigAction->setToolTip(tr("Edit the configuration file for DigitalKash"));
    editConfigExtAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit DigitalKash.conf (external)"), this);
    editConfigExtAction->setToolTip(tr("Edit the configuration file for DigitalKash (external editor)"));
    openDataDirAction = new QAction(QIcon(":/icons/folder"), tr("&Open data dir"), this);
    openDataDirAction->setToolTip(tr("Open the directory where DigitalKash data is stored"));
...
//void DigitalKashGUI::createMenuBar()
//{
//#ifdef Q_OS_MAC
...
void DigitalKashGUI::createToolBars()
{
    ...
    toolbar->setStyleSheet("QToolBar { background: none; } QToolButton { color: #ffffff; background: none; } QToolButton:hover { background-color: rgba(40, 152, 243, 0.4); } QToolButton:checked { background-color: rgba(56, 137, 202, 0.4); } QToolButton:pressed { background-color: rgba(100, 177, 240, 0.4); } #tabs { color: #ffffff; background: none; border: none;}");
    toolbar->setIconSize(QSize(64,64));

    if(!fUseDarkTheme)
    {
        toolbar->setStyleSheet("QToolBar { background: none; } QToolButton { color: #ffffff; background: none;} QToolButton:hover { background-color: rgba(40, 152, 243, 0.4); } QToolButton:checked { background-color: rgba(56, 137, 202, 0.4); } QToolButton:pressed { background-color: rgba(100, 177, 240, 0.4); } #tabs { color: #ffffff; background: none; border: none;}");//QToolBar { background: none; }
    }
...
{
    netLabel->setText("v1.1.0.0");// Version in GUI
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("DigitalKash client") + QString(" ") + tr("[testnet]"));
...
void DigitalKashGUI::setWalletModel(WalletModel *walletModel)
{
...
void DigitalKashGUI::setMessageModel(MessageModel *messageModel)
{
...
void DigitalKashGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("DigitalKash client"));
...
#ifndef Q_OS_MAC
void DigitalKashGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void DigitalKashGUI::optionsClicked()
{
...
void DigitalKashGUI::aboutClicked()
{
    AboutDialog dlg;
...
void DigitalKashGUI::aboutQtExt_Internal()
{
    aboutQtAction->triggered();
}

void DigitalKashGUI::aboutQtExt_Static()
{
    guiref->aboutQtExt_Internal();
}

void DigitalKashGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    //labelConnectionsIcon->setToolTip(tr("%n active connection(s) to DigitalKash network", "", count));
}

void DigitalKashGUI::setNumBlocks(int count)
{
    QString tooltip;
...
void DigitalKashGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    QString strTitle = tr("DigitalKash") + " - ";
...
void DigitalKashGUI::error(const QString &title, const QString &message, bool modal)
{
...
void DigitalKashGUI::changeEvent(QEvent *e)
{
...
void DigitalKashGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void DigitalKashGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        ...
        "Do you want to pay the fee?").arg(DigitalKashUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void DigitalKashGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
    ...
    notificator->notify(Notificator::Information,
                        (amount)<0 ? tr("Sent transaction") :
                          ...
                          tr("Date: %1\n"
                             ...
                             "Address: %4\n")
                          .arg(date)
                          .arg(DigitalKashUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                          .arg(type)
                          .arg(address), icon);
}

void DigitalKashGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void DigitalKashGUI::clearWidgets()
{
...
void DigitalKashGUI::gotoMasternodeManagerPage()
{
    ...
    setStyleSheet("#DigitalKash { background-image: url(:/images/background_light_alt); }");
}

void DigitalKashGUI::gotoBlockBrowser()
{
...
void DigitalKashGUI::gotoOverviewPage()
{
    ...
    setStyleSheet("#DigitalKash { background-image: url(:/images/background_light); }");
}

void DigitalKashGUI::gotoHistoryPage()
{
...
void DigitalKashGUI::gotoHistoryPage_static()
{
    guiref->gotoHistoryPage();
}

void DigitalKashGUI::gotoAddressBookPage()
{
...
void DigitalKashGUI::gotoReceiveCoinsPage()
{
...
void DigitalKashGUI::gotoSendCoinsPage()
{
...
void DigitalKashGUI::gotoSignMessageTab(QString addr)
{
...
void DigitalKashGUI::gotoVerifyMessageTab(QString addr)
{
...
void DigitalKashGUI::gotoMessagePage()
{
...
void DigitalKashGUI::gotoSettingsPage()
{
...
void DigitalKashGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void DigitalKashGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid DigitalKash address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void DigitalKashGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void DigitalKashGUI::setWalletUnlockStakingOnly()
{
...
void DigitalKashGUI::setUnencrypted()
{
    changePassphraseAction->setEnabled(false);
...
void DigitalKashGUI::setUnlocked()
{
...
void DigitalKashGUI::setEncryptionStatus(int status)
{
...
void DigitalKashGUI::encryptWallet()
{
    if(!walletModel)
        return;
...
void DigitalKashGUI::backupWallet()
{
...
void DigitalKashGUI::importPrivateKey()
{
...
void DigitalKashGUI::changePassphrase()
{
...
void DigitalKashGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void DigitalKashGUI::lockWallet()
{
    if(!walletModel)
        return;
...
void DigitalKashGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void DigitalKashGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void DigitalKashGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
/*void DigitalKashGUI::updateStakingIcon()
{
    updateWeight();
...
void DigitalKashGUI::detectShutdown()
{
    if (ShutdownRequested())
        QMetaObject::invokeMethod(QCoreApplication::instance(), "quit", Qt::QueuedConnection);
}

/*void DigitalKashGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void DigitalKashGUI::editConfig()
{
...
void DigitalKashGUI::editConfigExt()
{
...
void DigitalKashGUI::openDataDir()
{
