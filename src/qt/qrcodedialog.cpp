...
{
    QString ret = QString("DigitalKash:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DKASH unit input in lnReqAmount, we generate the URI with DKASH as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(DigitalKashUnits::format(DigitalKashUnits::DKASH, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
