...
/*!
  ...
  To invert the direction of an axis, use \ref setRangeDigitalKashersed.
*/
...
/*!
  ...
  upper part.
*/
void QCPAxis::setRangeDigitalKashersed(bool reversed)
{
  if (mRangeDigitalKashersed != reversed)
  {
    mRangeDigitalKashersed = reversed;
    mCachedMarginValid = false;
  }
}
...
double QCPAxis::coordToPixel(double value) const
{
  if (orientation() == Qt::Horizontal)
  {
    ...
    } else // mScaleType == stLogarithmic
    {
      if (value >= 0 && mRange.upper < 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeDigitalKashersed ? mAxisRect->right()+200 : mAxisRect->left()-200;
      else if (value <= 0 && mRange.upper > 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeDigitalKashersed ? mAxisRect->left()-200 : mAxisRect->right()+200;
      else
      {
        if (!mRangeDigitalKashersed)
  ...
  } else // orientation() == Qt::Vertical
  {
    ...
    {
      if (value >= 0 && mRange.upper < 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeDigitalKashersed ? mAxisRect->top()-200 : mAxisRect->bottom()+200;
      else if (value <= 0 && mRange.upper > 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeDigitalKashersed ? mAxisRect->bottom()+200 : mAxisRect->top()-200;
      else
      {
        if (!mRangeDigitalKashersed)
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  mAxisPainter->reversedEndings = mRangeDigitalKashersed;
...
/*!
  ...
  \li range reversed (\ref QCPAxis::setRangeDigitalKashersed)
...
void QCPAxisRect::setupFullAxesBox(bool connectRanges)
{
  ...
  xAxis2->setRangeDigitalKashersed(xAxis->rangeDigitalKashersed());
  ...
  yAxis2->setRangeDigitalKashersed(yAxis->rangeDigitalKashersed());
...
void QCPColorScaleAxisRectPrivate::draw(QCPPainter *painter)
{
  ...
  {
    mirrorHorz = mParentColorScale->mColorAxis.data()->rangeDigitalKashersed() && (mParentColorScale->type() == QCPAxis::atBottom || mParentColorScale->type() == QCPAxis::atTop);
    mirrorVert = mParentColorScale->mColorAxis.data()->rangeDigitalKashersed() && (mParentColorScale->type() == QCPAxis::atLeft || mParentColorScale->type() == QCPAxis::atRight);
  }
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  if (mAdaptiveSampling && dataCount >= maxCount) // use adaptive sampling only if there are at least two points per pixel on average
  {
    if (lineData)
    {
      ...
      int reversedFactor = keyAxis->rangeDigitalKashersed() ? -1 : 1; // is used to calculate keyEpsilon pixel into the correct direction
      int reversedRound = keyAxis->rangeDigitalKashersed() ? 1 : 0; // is used to switch between floor (normal) and ceil (reversed) rounding of currentIntervalStartKey
...
void QCPGraph::drawError(QCPPainter *painter, double x, double y, const QCPData &data) const
{
  ...
  if (keyAxis->orientation() == Qt::Vertical)
  {
    ...
    if (mErrorType == etKey || mErrorType == etBoth)
    {
      ...
      if (keyAxis->rangeDigitalKashersed())
        qSwap(a,b);
      // draw spine:
    ...
    if (mErrorType == etValue || mErrorType == etBoth)
    {
      ...
      if (valueAxis->rangeDigitalKashersed())
        qSwap(a,b);
      // draw spine:
      if (mErrorBarSkipSymbol)
      {
...
QPointF QCPGraph::lowerFillBasePoint(double lowerKey) const
{
  ...
  } else // valueAxis->mScaleType == QCPAxis::stLogarithmic
  {
    ...
    if (keyAxis->orientation() == Qt::Vertical)
    {
      if ((valueAxis->range().upper < 0 && !valueAxis->rangeDigitalKashersed()) ||
          (valueAxis->range().upper > 0 && valueAxis->rangeDigitalKashersed())) // if range is negative, zero is on opposite side of key axis
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  {
    // x is key
    // if an axis range is reversed, the data point keys will be descending. DigitalKasherse them, since following algorithm assumes ascending keys:
...
void QCPCurve::getCurveData(QVector<QPointF> *lineData) const
{
  ...
  double rectLeft = keyAxis->pixelToCoord(keyAxis->coordToPixel(keyAxis->range().lower)-strokeMargin*((keyAxis->orientation()==Qt::Vertical)!=keyAxis->rangeDigitalKashersed()?-1:1));
  double rectRight = keyAxis->pixelToCoord(keyAxis->coordToPixel(keyAxis->range().upper)+strokeMargin*((keyAxis->orientation()==Qt::Vertical)!=keyAxis->rangeDigitalKashersed()?-1:1));
  double rectBottom = valueAxis->pixelToCoord(valueAxis->coordToPixel(valueAxis->range().lower)+strokeMargin*((valueAxis->orientation()==Qt::Horizontal)!=valueAxis->rangeDigitalKashersed()?-1:1));
  double rectTop = valueAxis->pixelToCoord(valueAxis->coordToPixel(valueAxis->range().upper)-strokeMargin*((valueAxis->orientation()==Qt::Horizontal)!=valueAxis->rangeDigitalKashersed()?-1:1));
  int currentRegion;
...
void QCPBars::getVisibleDataBounds(QCPBarDataMap::const_iterator &lower, QCPBarDataMap::const_iterator &upperEnd) const
{
  ...
  while (it != mData->constBegin())
  {
    ...
    if (mKeyAxis.data()->orientation() == Qt::Horizontal)
      isVisible = ((!mKeyAxis.data()->rangeDigitalKashersed() && barBounds.right() >= lowerPixelBound) || (mKeyAxis.data()->rangeDigitalKashersed() && barBounds.left() <= lowerPixelBound));
    else // keyaxis is vertical
      isVisible = ((!mKeyAxis.data()->rangeDigitalKashersed() && barBounds.top() <= lowerPixelBound) || (mKeyAxis.data()->rangeDigitalKashersed() && barBounds.bottom() >= lowerPixelBound));
    if (isVisible)
      lower = it;
    else
      break;
  ...
  while (it != mData->constEnd())
  {
    ...
    if (mKeyAxis.data()->orientation() == Qt::Horizontal)
      isVisible = ((!mKeyAxis.data()->rangeDigitalKashersed() && barBounds.left() <= upperPixelBound) || (mKeyAxis.data()->rangeDigitalKashersed() && barBounds.right() >= upperPixelBound));
    else // keyaxis is vertical
      isVisible = ((!mKeyAxis.data()->rangeDigitalKashersed() && barBounds.bottom() >= upperPixelBound) || (mKeyAxis.data()->rangeDigitalKashersed() && barBounds.top() <= upperPixelBound));
    if (isVisible)
      upperEnd = it+1;
    else
      break;
...
void QCPBars::getPixelWidth(double key, double &lower, double &upper) const
{
  switch (mWidthType)
  {
    case wtAbsolute:
    {
      upper = mWidth*0.5;
      lower = -upper;
      if (mKeyAxis && (mKeyAxis.data()->rangeDigitalKashersed() ^ (mKeyAxis.data()->orientation() == Qt::Vertical)))
        qSwap(lower, upper);
      break;
    }
...
void QCPColorMap::updateLegendIcon(Qt::TransformationMode transformMode, const QSize &thumbSize)
{
  ...
  {
    bool mirrorX = (keyAxis()->orientation() == Qt::Horizontal ? keyAxis() : valueAxis())->rangeDigitalKashersed();
    bool mirrorY = (valueAxis()->orientation() == Qt::Vertical ? valueAxis() : keyAxis())->rangeDigitalKashersed();
