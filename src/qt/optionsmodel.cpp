...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", DigitalKashUnits::DKASH);
    ...
    if (!settings.contains("nAnonymizeDigitalKashAmount"))
        settings.setValue("nAnonymizeDigitalKashAmount", 1000);
    nAnonymizeDigitalKashAmount = settings.value("nAnonymizeDigitalKashAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeDigitalKashAmount"))
        SoftSetArg("-anonymizeDigitalKashamount", settings.value("nAnonymizeDigitalKashAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeDigitalKashAmount:
            return QVariant(nAnonymizeDigitalKashAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeDigitalKashAmount:
            nAnonymizeDigitalKashAmount = value.toInt();
            settings.setValue("nAnonymizeDigitalKashAmount", nAnonymizeDigitalKashAmount);
            emit AnonymizeDigitalKashAmountChanged(nAnonymizeDigitalKashAmount);
            break;
        default:
            break;
        }
