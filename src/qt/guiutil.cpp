...
{
    widget->setMaxLength(DigitalKashAddressValidator::MaxAddressLength);
    widget->setValidator(new DigitalKashAddressValidator(parent));
...
bool parseDigitalKashURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("DigitalKash"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!DigitalKashUnits::parse(DigitalKashUnits::DKASH, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseDigitalKashURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rev:// to DigitalKash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("rev://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "DigitalKash:");
    }
    QUrl uriInstance(uri);
    return parseDigitalKashURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DigitalKash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for DigitalKash.lnk
...
{
    return GetAutostartDir() / "DigitalKash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a DigitalKash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=DigitalKash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the DigitalKash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add DigitalKash app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DigitalKash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  DigitalKash-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("DigitalKash-Qt"));
...
void SetDarkThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QPushButton    { background: rgb(48,152,198); color: rgb(255,255,255); }"
                      ...
                      "QTabBar::tab   { color: rgb(194,194,194)); border: 1px solid rgb(78,79,83); border-bottom: none; padding: 0px; }"
                      "QTabBar::tab:selected  { background: rgb(41,44,48); }"
                      "QTabBar::tab:!selected { background: rgb(24,26,30); margin-top: 0px; }"
                      ...
                      "QTableWidget   { background: rgb(41,44,48); color: rgb(212,213,213); }"// TODO: Finish theme
                      "QTabWidget     { background: rgb(41,44,48); color: rgb(212,213,213); }"// TODO: Finish theme
